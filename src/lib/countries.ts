 ...
 *
 * Codes only: the browser already knows the names. Intl.DisplayNames turns each
 * into the user's own language, which is both smaller than shipping a name
 * table and correct for a reader who does not read English. Where it is
 * unavailable the code itself is shown, which is still unambiguous.
 */
...
/** Codes paired with names, ordered by name as the reader reads them. */
